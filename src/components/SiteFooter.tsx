...
export function SiteFooter({ lang, isHome = false }: SiteFooterProps) {
  ...
  return <footer className="footer"><div className="wrap"><div className="footer-top"><a className="brand" href={home}>nikvisuals<span className="brand-mark">.</span></a><div className="socials">{links.socials.filter((item) => item.platform !== 'facebook').map((item) => <a className="social-link" href={item.href} target="_blank" rel="noreferrer" key={item.label}><span><SocialIcon platform={item.platform} />{item.label}</span></a>)}</div></div><div className="footer-bottom"><span>{lang === 'de' ? 'Vorschau · Nicht zur Indexierung' : 'Preview · Not for indexing'}</span><a href={archive}>{lang === 'de' ? 'Videos' : 'Video work'}</a><a href={internship}>{lang === 'de' ? 'Praktikum' : 'Internships'}</a><a href={base + 'impressum/'}>{lang === 'de' ? 'Impressum' : 'Imprint'}</a><a href={base + 'datenschutz/'}>{lang === 'de' ? 'Datenschutz' : 'Privacy'}</a><button className="media-settings" type="button" onClick={openSettings}>{lang === 'de' ? 'Medien-Einstellungen' : 'Media settings'}</button></div></div></footer>;
}
