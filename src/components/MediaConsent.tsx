...
export function MediaConsentProvider({ children, lang }: { children: ReactNode; lang: Language }) {
  ...
  return <MediaContext.Provider value={value}>{children}{settings && <aside className="media-consent" aria-label={de ? 'Externe Medien' : 'External media'}><div><strong>{de ? 'Externe Medien' : 'External media'}</strong><p>{de ? 'YouTube wird erst nach deiner Erlaubnis und einem Klick geladen. Deine Auswahl wird nur lokal gespeichert.' : 'YouTube loads only after your permission and a click. Your choice is stored locally only.'}</p></div><div className="media-consent-actions"><button type="button" className="button button-quiet" onClick={() => choose(false)}>{de ? 'Nur notwendige' : 'Necessary only'}</button><button type="button" className="button button-accent" onClick={() => choose(true)}>{de ? 'YouTube erlauben' : 'Allow YouTube'}</button></div></aside>}{active && <div className="media-dialog-backdrop" role="presentation" onMouseDown={closePlayer}><section className="media-dialog" role="dialog" aria-modal="true" aria-label={active.title} onMouseDown={event => event.stopPropagation()}><div className="media-dialog-head"><strong>{active.title}</strong><button ref={closeButton} type="button" aria-label={de ? 'Video schließen' : 'Close video'} onClick={closePlayer}>×</button></div><div className="media-player"><iframe title={active.title} src={`https://www.youtube-nocookie.com/embed/${active.id}?autoplay=1`} allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div></section></div>}</MediaContext.Provider>;
}
